...
def plot_cylinder(center=[0,0,0], radius_x=1, radius_y=1, height=1, ax=None, color_surface = 'green', alpha=0.5, color_wireframe='black', line_wireframe=0.5, orientation=[1,1]):
    if ax is None:
        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')

    u = np.linspace(0, 2 * np.pi, 100)
    v = np.linspace(0, height, 50)
    U, V = np.meshgrid(u, v)

    x = radius_x * np.cos(U) * orientation[0] + center[0]
    y = radius_y * np.sin(U) * orientation[1] + center[1]
    z = V + center[2]

    ax.plot_surface(x, y, z, alpha=alpha, color=color_surface)
    ax.plot_wireframe(x, y, z, color=color_wireframe, linewidth=line_wireframe)
